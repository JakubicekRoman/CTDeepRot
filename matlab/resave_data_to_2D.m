...
path_save = ['C:\Data\Jakubicek\CTDeepRot_data\Datasets'];
mkdir(path_save)
...
paths = readcell('ListOfData.xlsx');

%%
DAT = '01';
numAll = size(lbl,1) + 1;

for pat = [1:size(paths,1)]
    Data = load_raw([paths{pat,1} '\' paths{pat,2}]);
    
    if size(Data,1)>300 && size(Data,2)>300 && size(Data,3)>300
        Data = imresize3(Data,[300,300,300]);
    end
 
pat
num = 1;

rot = [0,0,0;0,0,90;0,0,180;0,0,270;0,90,0;0,90,90;0,90,180;0,90,270;0,180,0;0,180,90;0,180,180;0,180,270;0,270,0;0,270,90;0,270,180;0,270,270;90,0,0;90,0,90;90,0,180;90,0,270;90,180,0;90,180,90;90,180,180;90,180,270];
...
for r = 1:24
%        data = imrotate3(data1,rot(r,1),[1,0,0],'nearest');
%        data = imrotate3(data,rot(r,2),[0,1,0],'nearest'); % rotY - rotace podle X osy
%        data = imrotate3(data,rot(r,3),[0,0,1],'nearest'); % rotZ - rotace AXIAL   
       
        data = rot90_3D(Data, 1, rot(r,1)/90);
        data = rot90_3D(data, 2, rot(r,2)/90);
        data = rot90_3D(data, 3, rot(r,3)/90);
...
%                     img = single(squeeze(max(data(ceil(size(data,2)./2)-p:ceil(size(data,2)./2)+p,:,:),[],1)));
                ...
                %% ground truth
                    lbl{numAll,1} = name;
                    lbl{numAll,2} = rot(r,1);
                    lbl{numAll,3} = rot(r,2);
                    lbl{numAll,4} = rot(r,3);
%                     lbl{numAll,5} = permut;
